...
const topNav = [
  ...
  { to: "/live-telemetry", label: "Telemetrie Live", icon: RadioTower },
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const { unreadAlerts } = usePulseStore();
    ...
    <div className="relative min-h-screen text-foreground">
      ...
      <header className="sticky top-0 z-40 border-b border-border/60 bg-background/55 backdrop-blur-xl">
        <div className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 lg:px-6 h-16 min-w-0">
          <Link
            to="/"
            className="flex items-center gap-2.5 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60 rounded-lg"
          >
          ...
          <nav className="flex flex-col gap-0.5 mt-1">
            {sideNav.map((item) => {
              ...
              return (
                ...
                >
                  {active && (
                    <span className="absolute left-0 top-1/2 -translate-y-1/2 h-6 w-0.5 rounded-full bg-gradient-to-b from-[var(--cyan-glow)] to-[var(--indigo-glow)]" />
                  )}
                  <Icon
                    className={cn(
...
function ProfilMenu() {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger
        className="group flex items-center gap-2 rounded-full pl-1 pr-2 py-1 hover:bg-secondary/50 transition focus:outline-none focus:ring-2 focus:ring-ring/50"
      ...
      >
        <span className="relative grid h-10 w-10 shrink-0 place-items-center rounded-full ring-2 ring-background/60 transition-all duration-300 group-hover:ring-[var(--cyan-glow)]/80 group-hover:shadow-[0_0_22px_-2px_oklch(0.78_0.18_210/0.75)]">
          ...
          />
          <span
            className="absolute bottom-0 right-0 h-3 w-3 rounded-full bg-success ring-2 ring-background animate-pulse-soft"
            aria-label="Online"
          />
      ...
      >
        <DropdownMenuLabel className="flex items-center gap-3 py-2.5 px-2">
          <span className="relative">
            <img
              ...
              width={48}
              height={48}
              className="avatar-swap h-12 w-12 rounded-full object-cover object-center ring-2 ring-[var(--cyan-glow)]/40"
            />
        ...
        >
          <UserCircle className="h-4 w-4 text-muted-foreground" /> Profil
        </DropdownMenuItem>
            ...
            <DropdownMenuSubContent className="glass-strong border-border/60 w-60 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:slide-in-from-left-2">
              {UNITS.map((u) => (
                <DropdownMenuItem
                  ...
                  onSelect={() => {
                    setUnit(u as Unit);
                    toast.success("Unitatea medicala a fost actualizata", { description: u });
                  }}
                  className="gap-2 cursor-pointer"
                >
                  {u === unit ? (
                    <Check className="h-3.5 w-3.5 text-[var(--cyan-glow)]" />
                  ) : (
        ...
        <DropdownMenuSub>
          <DropdownMenuSubTrigger className="gap-2">
            <Stethoscope className="h-4 w-4 text-muted-foreground" /> Schimba Doctorul /
            Coordonatorul
          </DropdownMenuSubTrigger>
          <DropdownMenuPortal>
            <DropdownMenuSubContent className="glass-strong border-border/60 w-72 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:slide-in-from-left-2">
              {COORDINATORS.map((c) => (
                <DropdownMenuItem
                  key={c.name}
                  onSelect={() => {
                    setCoordinator(c.name as Coordinator);
                    toast.success("Coordonator actualizat", { description: c.name });
                  }}
            ...
            <DropdownMenuSubContent className="glass-strong border-border/60 w-56 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:slide-in-from-left-2">
              {(
                ...
                <DropdownMenuItem
                  ...
                  onSelect={() => {
                    setTheme(id);
                    toast.success("Tema actualizata", { description: label });
                  }}
                  className="gap-2 cursor-pointer"
                >
                  ...
                  <span>{label}</span>
                  {theme === id && (
                    <Check className="ml-auto h-3.5 w-3.5 text-[var(--cyan-glow)]" />
                  )}
                </DropdownMenuItem>
              ))}
            </DropdownMenuSubContent>
        ...
        <DropdownMenuItem
          ...
          className="gap-2 cursor-pointer"
        >
          <BellDot className="h-4 w-4 text-muted-foreground" /> Setari Notificari
        </DropdownMenuItem>
        <DropdownMenuItem
          onSelect={(e) => {
            e.preventDefault();
          ...
          className="gap-2 cursor-pointer"
        >
          <SlidersHorizontal className="h-4 w-4 text-muted-foreground" /> Preferinte Prognoza
        </DropdownMenuItem>
        <DropdownMenuItem
          onSelect={(e) => {
            e.preventDefault();
          ...
          className="gap-2 cursor-pointer"
        >
          <Settings className="h-4 w-4 text-muted-foreground" /> Setari Cont
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem
          onSelect={() =>
    ...
    <div className="hidden md:flex items-center gap-2 px-4 lg:px-6 h-9 border-t border-border/40 bg-background/30 text-[11px] text-muted-foreground">
      ...
      {active && (
        <span className="inline-flex items-center gap-1.5 rounded-full border border-[var(--cyan-glow)]/40 bg-[var(--cyan-glow)]/10 px-2 py-0.5 animate-fade-up">
          ...
          <span className="text-foreground/90 font-medium truncate max-w-[280px]">
            Scenariu Activ: {active.name}
          </span>
          <span className="text-muted-foreground hidden lg:inline">
            - {active.riskLevel} - {active.riskScore}/100
          </span>
          <button
            onClick={() => setActive(null)}
            className="ml-1 text-muted-foreground hover:text-foreground transition"
            aria-label="Sterge scenariul activ"
          >
            x
          </button>
        </span>
      ...
      <span className="ml-auto inline-flex items-center gap-1.5">
        ...
        Flux live de semnale - {telemetry.connectionMode === "websocket"
          ? "WebSocket"
          : "fallback"}{" "}
        - risc mediu {telemetry.avgBurnout}/100
      </span>
    </div>
  );
}
