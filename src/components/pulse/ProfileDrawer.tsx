...
const themeLabels: Record<Theme, string> = { light: "Luminos", dark: "Intunecat", system: "Sistem", "high-contrast": "Contrast ridicat" };
...
export function ProfilDrawer({ open, onOpenChange }: { open: boolean; onOpenChange: (o: boolean) => void }) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="glass-strong border-l border-border/60 w-full sm:max-w-md overflow-y-auto scrollbar-thin p-0">
        <div className="relative p-6 border-b border-border/40 bg-gradient-to-br from-[var(--cyan-glow)]/10 to-[var(--indigo-glow)]/10">
          ...
          <div className="mt-5 flex items-center gap-4">
            ...
            <button
              onClick={() => { setEditing((e) => !e); toast.success(editing ? "Modificarile au fost salvate" : "Editezi profilul"); }}
        ...
        <Section title="Spatiu de lucru">
          <div className="grid grid-cols-1 gap-2">
            ...
            <select
              value={unit}
              onChange={(e) => { setUnit(e.target.value as typeof UNITS[number]); toast.success("Unitatea a fost actualizata"); }}
        ...
        <Section title="Preferinte notificari">
          <Toggle label="Alerte critice de epuizare" defaultOn />
          <Toggle label="Rezumat zilnic de prognoza" defaultOn />
          <Toggle label="Finalizare scenariu" defaultOn={false} />
          <Toggle label="Raport saptamanal pentru conducere" defaultOn />
        </Section>
            ...
            {(["light", "dark", "system", "high-contrast"] as Theme[]).map((t) => {
              ...
              return (
                <button
                  ...
                  className={"inline-flex items-center gap-2 rounded-lg border px-3 py-2 text-xs font-medium transition " + (active ? "border-[var(--cyan-glow)]/60 bg-[var(--cyan-glow)]/10 text-foreground" : "border-border/60 bg-background/30 text-muted-foreground hover:text-foreground hover:bg-secondary/40")}
                >
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return <div className="p-5 border-b border-border/40 animate-fade-up"><div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground mb-3">{title}</div><div className="space-y-2">{children}</div></div>;
}

function Row({ icon: Icon, label, value }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string }) {
  return <div className="flex items-center gap-3 rounded-lg bg-secondary/30 px-3 py-2"><Icon className="h-3.5 w-3.5 text-[var(--cyan-glow)]" /><div className="flex-1 min-w-0"><div className="text-[11px] text-muted-foreground">{label}</div><div className="text-xs font-medium truncate">{value}</div></div></div>;
}

function Toggle({ label, defaultOn }: { label: string; defaultOn: boolean }) {
  const [on, setOn] = useState(defaultOn);
  return <button onClick={() => { setOn(!on); toast.success(`${label} ${!on ? "activata" : "dezactivata"}`); }} className="w-full flex items-center justify-between rounded-lg bg-secondary/30 px-3 py-2.5 text-xs hover:bg-secondary/50 transition"><span>{label}</span><span className={"relative inline-flex h-5 w-9 rounded-full transition " + (on ? "bg-[var(--cyan-glow)]/80" : "bg-muted")}><span className={"absolute top-0.5 h-4 w-4 rounded-full bg-background shadow transition-all " + (on ? "left-4" : "left-0.5")} /></span></button>;
}

function ActivityRow({ text, time }: { text: string; time: string }) {
  return <div className="flex items-start gap-3 rounded-lg bg-secondary/20 px-3 py-2"><span className="mt-1.5 h-1.5 w-1.5 rounded-full bg-[var(--cyan-glow)]" /><div className="flex-1 min-w-0"><div className="text-xs">{text}</div><div className="text-[10px] text-muted-foreground">{time}</div></div></div>;
}

function ReportRow({ title, date }: { title: string; date: string }) {
  return <div className="flex items-center gap-3 rounded-lg bg-secondary/20 px-3 py-2 hover:bg-secondary/40 transition cursor-pointer"><FileText className="h-3.5 w-3.5 text-[var(--indigo-glow)]" /><div className="flex-1 min-w-0"><div className="text-xs font-medium truncate">{title}</div><div className="text-[10px] text-muted-foreground">{date}</div></div></div>;
}
