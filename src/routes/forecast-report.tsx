  ...
  head: () => ({
    meta: [
      { title: "Raport Prognoza - PulseGuard AI" },
      {
        name: "description",
        content:
          "Raport operational AI pentru risc de epuizare, prognoza pe 14 zile si actiuni recomandate.",
      },
      { property: "og:title", content: "Raport Prognoza - PulseGuard AI" },
      {
        property: "og:description",
        content: "Raport operational pentru personal, ture si interventii.",
      },
    ],
  }),
...
const defaultDrivers = [
  {
    id: "overtime",
    label: "Ore suplimentare acumulate",
    severity: 88,
    icon: Flame,
    tone: "danger",
    explain:
      "Orele suplimentare au crescut in ultimele 14 zile si mai multi oameni depasesc pragul de 12 ore pe saptamana.",
    mitigation:
      "Limiteaza orele suplimentare la 8 ore pe saptamana si muta sarcina catre personal de rezerva.",
  },
  {
    id: "nights",
    label: "Ture de noapte consecutive",
    severity: 81,
    icon: MoonStar,
    tone: "danger",
    explain:
      "Mai multe persoane au lucrat cel putin 4 nopti consecutive, ceea ce reduce recuperarea si creste oboseala.",
    mitigation: "Adauga o pauza obligatorie de 36 ore dupa 3 ture de noapte consecutive.",
  },
  {
    id: "load",
    label: "Volum mare de pacienti",
    severity: 74,
    icon: Users,
    tone: "warning",
    explain: "Raportul pacienti personal a crescut, mai ales in ATI si UPU.",
    mitigation:
      "Redistribuie pacientii eligibili si adauga personal senior in intervalele cu presiune mare.",
  },
  {
    id: "recovery",
    label: "Timp redus de recuperare",
    severity: 67,
    icon: BatteryLow,
    tone: "warning",
    explain: "Timpul median intre ture a scazut sub pragul recomandat de 11 ore.",
    mitigation: "Blocheaza programarile care lasa mai putin de 11 ore intre ture.",
  },
  {
    id: "absence",
    label: "Concedii medicale in crestere",
    severity: 58,
    icon: UserMinus,
    tone: "warning",
    explain: "Absentele cresc presiunea pe personalul ramas in tura.",
    mitigation: "Activeaza personalul de rezerva si verifica zilnic zonele cu lipsuri.",
  },
] as const;
...
function ReportPage() {
  ...
  const risk = active?.riskScore ?? (telemetry.staffCount ? telemetry.avgBurnout : 78);
  ...
  const followUps = active?.followUpIndicators.length
  ...
  const comparison = [
    {
      metric: "Risc epuizare",
      current: `${risk} / 100`,
      recommended: `${Math.max(20, risk - 16)} / 100`,
      impact: "-16 puncte",
      positive: true,
    },
    {
      metric: "Ore suplimentare",
      current: "14.2 h/sapt",
      recommended: "8.0 h/sapt",
      impact: "-44%",
      positive: true,
    },
    {
      metric: "Presiune personal",
      current: `${pressure} / 100`,
      recommended: `${Math.max(20, pressure - 14)} / 100`,
      impact: "-14 puncte",
      positive: true,
    },
    {
      metric: "Risc prognozat",
      current: `${predicted} / 100`,
      recommended: `${Math.max(20, predicted - 18)} / 100`,
      impact: "-18 puncte",
      positive: true,
    },
    {
      ...
      current: `${urgency} / 100`,
      recommended: `${Math.max(20, urgency - 24)} / 100`,
      impact: "-24 puncte",
      positive: true,
    },
  ];
    ...
    setTimeout(
      () => {
        ...
        toast.success("Raport regenerat", {
          ...
          description: "Prognoza si planul au fost actualizate.",
        });
      },
  ...
  const exportPdf = async () => {
    try {
      ...
      toast.error("Exportul PDF a esuat", {
        description: "Incearca din nou peste cateva secunde.",
      });
    }
  };
    ...
    try {
      if (navigator.share) {
        ...
        toast.success("Raport copiat", {
          description: "Textul raportului a fost copiat in clipboard.",
        });
      }
    } catch {
  ...
  const saveScenario = () => {
    if (active) persistScenario(active);
    saveReport({
      title: active?.name ?? "Raport prognoza risc epuizare",
      department: effectiveUnit,
      coordinator: effectiveCoordinator,
    ...
    toast.success(active ? "Scenariu salvat" : "Raport salvat", {
      description: active?.name ?? "Raportul curent a fost salvat local.",
    });
  };
    ...
    <AppShell>
      <header className="mb-5 animate-stagger">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Analiza operationala completa a presiunii pe personal, cu proiectie pe 14 zile si masuri
          clare pentru urmatoarele 72 de ore.
        </p>
      </header>

      <div
        className="sticky top-16 z-30 -mx-1 mb-6 animate-stagger"
      ...
      >
        <div className="glass luminous-border rounded-2xl px-4 py-3 flex flex-wrap items-center gap-x-5 gap-y-2">
          ...
          <Meta
            label="Generat"
            value={<span suppressHydrationWarning>{stamp || "chiar acum"}</span>}
          />
          <Divider />
          ...
          <div className="ml-auto flex flex-wrap gap-2">
            ...
            <button
              onClick={exportPdf}
              className="btn-glow inline-flex items-center gap-1.5 rounded-lg border border-border/60 bg-secondary/40 px-3 py-1.5 text-xs hover:bg-secondary/70 transition"
            >
                ...
                return (
                  <li
                    ...
                    className={cn(
                      ...
                      !done && !activeStep && "opacity-50",
                    )}
                  >
                    {done ? (
                      <CheckCircle2 className="h-3.5 w-3.5 text-success" />
                    ) : activeStep ? (
                      <Loader2 className="h-3.5 w-3.5 text-[var(--cyan-glow)] animate-spin" />
                    ) : (
        ...
        <article className="glass luminous-border rounded-2xl p-5">
          <div className="flex items-start gap-3">
            ...
            <p className="text-sm lg:text-base leading-relaxed text-foreground/90">
              {active?.explanation ??
                "Prognoza indica risc ridicat de epuizare in urmatoarele 14 zile. Semnalele principale sunt orele suplimentare, turele de noapte consecutive si presiunea crescuta pe personal. Prima masura recomandata este redistribuirea sarcinii si activarea personalului de rezerva pentru urmatoarele 72 de ore."}
            </p>
          </div>
      ...
      <Section
        eyebrow="Semnale istorice"
        title="Presiune operationala in ultimele 30 zile"
        delay={0.18}
      >
      ...
      <Section eyebrow="Prognoza predictiva" title="Prognoza risc epuizare pe 14 zile" delay={0.26}>
        <div className="glass luminous-border rounded-2xl p-5">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            Linia continua arata valorile observate. Linia punctata arata prognoza. Banda umbrita
            arata intervalul de incredere.
          </p>
        </div>
      </Section>
        ...
        <ol className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
          {drivers.map((d, i) => {
            ...
            return (
              <li
                key={d.id}
                className="glass luminous-border rounded-2xl p-4 animate-stagger hover-lift"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-2 text-[11px] text-muted-foreground inline-flex items-start gap-1.5">
                      ...
                      <span>
                        <span className="text-foreground/80 font-medium">Masura:</span>{" "}
                        {d.mitigation}
                      </span>
                    </p>
                  </div>
        ...
        <div className="glass luminous-border rounded-2xl p-5">
          <ol className="flex flex-col gap-2.5">
            {actions.map((a, i) => (
              <li
                key={a}
                className="flex items-start gap-3 rounded-xl border border-border/60 bg-secondary/30 p-3 animate-stagger hover:border-[var(--cyan-glow)]/40 transition"
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {comparison.map((row) => (
                  <tr
                    key={row.metric}
                    className="border-b border-border/40 last:border-0 hover:bg-secondary/20 transition"
                  >
      ...
      <footer className="mt-8 mb-2 flex items-start gap-2 rounded-xl border border-border/60 bg-secondary/20 p-3 animate-fade-up">
        ...
        <p className="text-[11px] text-muted-foreground leading-relaxed">
          Acest raport este suport decizional operational. Nu este instrument de diagnostic medical.
          Deciziile finale raman la echipa clinica si operationala calificata.
        </p>
      </footer>
    </AppShell>
  );
...
function Divider() {
  return <span className="hidden md:block h-6 w-px bg-border/60" />;
}
