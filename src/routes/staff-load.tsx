...
export const Route = createFileRoute("/staff-load")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Presiune de lucru si ocupare pentru fiecare sectie a spitalului.",
      },
      { property: "og:title", content: "Incarcare Personal - PulseGuard AI" },
      {
        property: "og:description",
        content: "Incarcare de personal si ocupare la nivel de sectie.",
      },
    ],
  }),
    ...
    <AppShell>
      <header className="mb-6 animate-fade-up">
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground inline-flex items-center gap-2">
          <Users className="h-3 w-3" /> Incarcare Personal
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {departments.map((d, i) => {
          ...
          return (
            <article
              key={d.id}
              className="glass luminous-border rounded-2xl p-5 animate-fade-up"
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
                <span>
                  {d.staff} angajati - {d.occupancy}% ocupare
                </span>
                <span
                  className={cn(
                ...
                >
                  <Trend className="h-3 w-3" /> {d.delta > 0 ? "+" : ""}
                  {d.delta}%
                </span>
              </div>
            </article>
