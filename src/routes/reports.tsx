...
export const Route = createFileRoute("/reports")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Toate rapoartele generate pentru riscul de epuizare pe sectii.",
      },
      { property: "og:title", content: "Rapoarte - PulseGuard AI" },
      {
        property: "og:description",
        content: "Biblioteca de rapoarte generate de AI pentru personal.",
      },
    ],
  }),
...
function RapoartePage() {
  const { savedReports, telemetry } = usePulseStore();
  return (
    <AppShell>
      <header className="mb-6 animate-fade-up">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Ultimele rapoarte de prognoza si interventie generate de AI pentru fiecare unitate
          medicala.
        </p>
      </header>
      ...
      {savedReports.length > 0 && (
        <section className="mb-6 animate-fade-up">
          <h2 className="mb-3 text-sm font-semibold">Rapoarte salvate local</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            {savedReports.map((report) => {
              const band = riskBand(report.riskScore);
              return (
                <Link
                  key={report.id}
                  to="/forecast-report"
                  className="group glass luminous-border rounded-2xl p-5 hover:-translate-y-0.5 transition-transform"
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] uppercase tracking-wider rounded-full px-2 py-0.5 border border-[var(--cyan-glow)]/40 text-[var(--cyan-glow)]">
                      {report.source}
                    </span>
                  </div>
                  <h3 className="mt-3 text-base font-semibold">{report.title}</h3>
      ...
      <section className="mb-6 glass luminous-border rounded-2xl p-4 animate-fade-up">
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">
          Context telemetrie live
        </div>
        <p className="mt-1 text-sm">
          Risc mediu live {telemetry.avgBurnout}/100, {telemetry.staffCount} digital twins active,
        ...
        {departments.map((d, i) => {
          ...
          return (
            <Link
              key={d.id}
              to="/forecast-report"
              className="group glass luminous-border rounded-2xl p-5 animate-fade-up hover:-translate-y-0.5 transition-transform"
            ...
            >
              ...
              <div className="mt-1 h-1.5 rounded-full bg-secondary/60 overflow-hidden">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-[var(--cyan-glow)] via-warning to-danger"
                  style={{ width: `${d.burnoutRisk}%` }}
                />
              </div>
